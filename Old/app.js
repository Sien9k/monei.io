...
// Helper: format tanggal hari ini ke string YYYY-MM-DD
function getTanggalHariIni() {
    const d = new Date();
    return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

// Helper: safe parseFloat — bisa handle angka biasa maupun string format "1.000.000"
function safeFloat(val) {
    if (val === null || val === undefined) return 0;
    // Hapus titik ribuan agar tidak dianggap desimal
    const clean = String(val).replace(/\./g, '').replace(',', '.');
    const n = parseFloat(clean);
    return isNaN(n) ? 0 : n;
}

// Helper: format input nominal saat user mengetik — tampilkan titik ribuan otomatis
...
async function hitungSisaBudget() {
    ...
    const totalBudget = safeFloat(budget.amount);
    
    const hariIniStr = getTanggalHariIni();
    ...
    tampilan.innerHTML = `
        <div style="display: flex; flex-direction: column; align-items: center; padding: 5px 0; font-family: 'Comfortaa', sans-serif;">
...
async function downloadPDF() {
    ...
    if (txs) {
        txs.forEach(t => {
            ...
            doc.text(amt.toLocaleString('id-ID'), 70, y);
...
async function muatKategori() {
    ...
    mapKategori = {};
    // FIX PENTING: Menambahkan resetBudgetKategori agar menunya terisi
...
document.getElementById('formTransaksi').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const amount = safeFloat(amountRaw);
...
document.getElementById('formBudget').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        // FIX PENTING: Membersihkan titik ribuan dari input Set Anggaran sebelum dikirim
        const rawAmt = document.getElementById('budgetAmount').value;
        const cleanAmt = safeFloat(rawAmt);
...
async function inisialisasiAplikasi() {
    ...
    // Menerapkan titik ribuan di input Transaksi
    const amountEl = document.getElementById('amount');
    if (amountEl) {
        amountEl.type = 'text';
        amountEl.inputMode = 'numeric';
        amountEl.addEventListener('input', () => formatInputAmount(amountEl));
    }   

    // Menerapkan titik ribuan di input Set Anggaran (BARU)
    const budgetAmountEl = document.getElementById('budgetAmount');
    if (budgetAmountEl) {
        budgetAmountEl.type = 'text';
        budgetAmountEl.inputMode = 'numeric';
        budgetAmountEl.addEventListener('input', () => formatInputAmount(budgetAmountEl));
    }
}

// ==========================================
// 8. FUNGSI RESET BUDGET 
// ==========================================
async function resetAnggaranTarget() {
    try {
        const katId = document.getElementById('resetBudgetKategori').value;
        if (!katId) {
            return alert("Silakan pilih kategori yang ingin di-reset terlebih dahulu!");
        }
        ...
        if (confirm("Apakah Anda yakin ingin me-reset (menghapus) target anggaran untuk kategori ini?")) {
            ...
            document.getElementById('resetBudgetKategori').value = '';
            alert("✅ Budget untuk kategori tersebut berhasil di-reset menjadi kosong!");
        }
    } catch (err) {
        alert("GAGAL MENGERJAKAN RESET BUDGET:\n" + err.message);
    }
}
...
window.bukaRiwayatDenganShortcut = bukaRiwayatDenganShortcut;
window.setPresetPeriodeTanggal = setPresetPeriodeTanggal;
...
window.resetAnggaranTarget = resetAnggaranTarget; // Mendaftarkan tombol Reset Budget
